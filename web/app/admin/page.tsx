...
// Tipo para a Igreja (ATUALIZADO COM OS DADOS DE REDE)
...
export default function AdminPage() {
    ...
    const handleCreateChurch = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (error.code === 'auth/email-already-in-use') {
                alert("⚠️ ERRO: Este e-mail já está cadastrado.");
            } else {
    ...
    const handleUpgradePlan = async (church: ChurchData) => {
        ...
        if (confirm(`Mudar o limite da igreja "${church.name}" para ${newLimit > 400 ? 'ILIMITADO' : newLimit + ' membros'}?`)) {
            try {
                await updateDoc(doc(db, "churches", church.id), { plan: newPlan, planLimit: newLimit });
                fetchChurches();
    ...
    const handleUpgradeModules = async (church: ChurchData) => {
        ...
        try {
            await updateDoc(doc(db, "churches", church.id), { planModules: newModules });
            fetchChurches();
        } catch (error) { alert("Erro ao atualizar módulos."); }
    };

    const toggleTestFlag = async (church: ChurchData) => {
        const newValue = !church.isTest;
        if (!confirm(`Deseja alterar o status de teste de "${church.name}"?`)) return;
        try {
            await updateDoc(doc(db, "churches", church.id), { isTest: newValue });
            fetchChurches();
        } catch (error) { alert("Erro ao atualizar flag de teste."); }
    };

    const deleteChurch = async (id: string) => {
        if (confirm("⚠️ ATENÇÃO: Isso vai apagar os DADOS da igreja.\nDeseja continuar?")) {
            try {
    ...
    const saveNetworkConfig = async () => {
        ...
        try {
            // Se for Sede, não pode ter parentId. Se for filial, não pode ser sede.
            ...
            alert("Erro ao salvar configuração de rede.");
            console.error(error);
        }
    };
        ...
        <div className="p-4 md:p-8 min-h-screen pb-24 bg-gray-50">
            ...
            <div className="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
                <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex items-center gap-4">
                    ...
                    <div>
                        <p className="text-xs font-bold text-gray-400 uppercase tracking-wider">Igrejas</p>
                ...
                <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex items-center gap-4">
                    ...
                    <div>
                        <p className="text-xs font-bold text-gray-400 uppercase tracking-wider">Membros</p>
                ...
                <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex items-center gap-4">
                    ...
                    <div>
                        <p className="text-xs font-bold text-gray-400 uppercase tracking-wider">Por País</p>
                    ...
                    <form onSubmit={handleCreateChurch} className="p-6 space-y-4">
                        ...
                        <div>
                            ...
                            <select value={newChurch.plan} onChange={e => handlePlanChangeSelect(e.target.value)} className="w-full p-3 border border-gray-200 rounded-xl text-sm font-bold bg-gray-50">
                                <option value="congr">Congregação (Até 100)</option>
                                <option value="sede">Sede (Até 400)</option>
                                <option value="min">Ministério (Ilimitado)</option>
                            </select>
                        </div>
                        ...
                        <button type="submit" disabled={loading} className="w-full py-3 rounded-xl font-bold text-white bg-blue-600 hover:bg-blue-700 shadow-lg shadow-blue-200 transition">
                ...
                <div className="lg:col-span-2">
                    <div className="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden min-h-[500px]">
                        ...
                        <div className="divide-y divide-gray-100">
                            {churches.length === 0 ? (
                                <div className="p-8 text-center text-gray-400 flex flex-col items-center"><Search size={40} className="mb-2 opacity-20" /><p>Nenhuma igreja.</p></div>
                            ) : (
                                churches.map((church) => {
                                    // Pega o nome da Sede, caso essa igreja seja filial
                                    ...
                                    return (
                                        <div key={church.id} className="p-4 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 hover:bg-gray-50 transition">
                                                ...
                                                <div className="min-w-0">
                                                    ...
                                                    <div className="mt-2 flex items-center gap-2 flex-wrap">
                                                        {church.isHeadquarters && (
                                                            <span className="text-[9px] uppercase font-bold px-2 py-0.5 rounded-full bg-indigo-100 text-indigo-700 border border-indigo-200 flex items-center gap-1">
                                                                <Building2 size={10}/> Sede Principal
                                                            </span>
                                                        )}
                                                        {church.parentId && (
                                                            <span className="text-[9px] uppercase font-bold px-2 py-0.5 rounded-full bg-cyan-100 text-cyan-700 border border-cyan-200 flex items-center gap-1" title={`Pertence à sede: ${parentChurch?.name || 'Desconhecida'}`}>
                                                                <MapPin size={10}/> Filial: {parentChurch?.name?.substring(0,15) || 'Sede'}...
                                                            </span>
                                                        )}
                                                        ...
                                                        {church.isTest && (
                                                            <span className="text-[9px] uppercase font-bold px-2 py-0.5 rounded-full bg-yellow-100 text-yellow-700">Teste</span>
                                                        )}
                                                    </div>
                                                </div>
                                            </div>
                                            ...
                                            <div className="flex flex-wrap items-center gap-2 w-full sm:w-auto">
                                                ...
                                                <button onClick={() => handleUpgradePlan(church)} className="px-3 py-2 rounded-lg text-xs font-bold text-purple-600 bg-purple-50 hover:bg-purple-100" title="Atualizar Limite"><Crown size={14} /></button>
                                                <button onClick={() => handleUpgradeModules(church)} className="px-3 py-2 rounded-lg text-xs font-bold text-blue-600 bg-blue-50 hover:bg-blue-100" title="Mudar Módulos"><Building2 size={14} /></button>
                                                <button onClick={() => toggleTestFlag(church)} className="px-3 py-2 rounded-lg text-xs font-bold text-gray-600 bg-gray-100 hover:bg-gray-200" title="Teste"><Flag size={14} /></button>
                                                <button onClick={() => toggleStatus(church)} className={`px-3 py-2 rounded-lg text-xs font-bold border ${church.status === 'active' ? 'text-gray-500 hover:bg-red-50 hover:text-red-600' : 'bg-green-600 text-white'}`}>
                                                    {church.status === 'active' ? <Ban size={14} /> : <Check size={14} />}
                                                </button>
                                                <button onClick={() => deleteChurch(church.id)} className="px-3 py-2 rounded-lg text-gray-300 hover:bg-red-100 hover:text-red-600"><Trash2 size={16} /></button>
                                            </div>
                                        </div>
                                    );
                                })
            ...
            {/* MODAL DE GESTÃO DE REDE (MATRIZ / FILIAL) */}
                        ...
                        <div className="p-6 space-y-6">
                            ...
                            <label className={`flex items-start gap-4 p-4 rounded-xl border-2 cursor-pointer transition ${editIsHq ? 'border-indigo-600 bg-indigo-50' : 'border-gray-200 hover:bg-gray-50'}`}>
                                <input 
                                    ...
                                    onChange={(e) => {
                                        ...
                                        if (e.target.checked) setEditParentId(""); // Sede não pode ter parent
                                    }}
                            ...
                            <div className={`p-4 rounded-xl border-2 transition ${!editIsHq && editParentId ? 'border-cyan-500 bg-cyan-50' : 'border-gray-200'}`}>
                                ...
                                >
                                    ...
                                    {headquartersList
                                        .filter(hq => hq.id !== networkModal.church?.id) // Não pode ser filial de si mesma
                                        .map(hq => (
