...
import { 
  ...
  MapPin, Phone, Mail, ChevronLeft, ChevronRight, Loader2, HandCoins, Lock, X, Building2, Heart, Briefcase, Camera, ShieldCheck 
} from "lucide-react";
...
export default function MembersPage() {
  ...
  const handlePrintExecute = () => {
    ...
    const translateMarital = (st: string | null | undefined) => {
        if(st === 'married') return 'Casado(a)';
        if(st === 'divorced') return 'Divorciado(a)';
        if(st === 'widowed') return 'Viúvo(a)';
        return 'Solteiro(a)';
    };
        ...
        <tr style="border-bottom: 1px solid #eee;">
            ...
            <td style="padding: 8px;">
                <div style="display: flex; align-items: center; gap: 10px;">
                    ${m.photoUrl ? `<img src="${m.photoUrl}" style="width: 30px; height: 30px; border-radius: 50%; object-fit: cover;" />` : ''}
                    <strong>${m.fullName}</strong>
                </div>
            </td>
            <td style="padding: 8px;">${m.role === 'admin' ? 'Pastor' : m.role}</td>
            <td style="padding: 8px;">${m.phone || '-'}</td>
            <td style="padding: 8px;">${translateMarital(m.maritalStatus)}</td>
    ...
    const html = `
        <html>
            ...
            <body>
                ${logoHtml}
                <h1>${churchName}</h1>
                <p>Relatório Geral • ${today} • Total: ${filteredMembers.length}</p>
                <table>
                    <thead><tr><th width="30">#</th><th>Nome</th><th>Cargo</th><th>Telefone</th><th>Est. Civil</th><th>Status</th></tr></thead>
                    <tbody>${rows}</tbody>
                </table>
  ...
  const handleOpenModal = (member?: Member) => {
    if (member) {
      ...
      setFormData({
        ...
        gender: member.gender || "male", // Correção simples para lidar com null
        maritalStatus: member.maritalStatus || "single", // Correção simples para lidar com null
    ...
    <div className="p-4 md:p-8 min-h-screen bg-gray-50 pb-24">
      <div className="max-w-6xl mx-auto mb-6 flex flex-col md:flex-row justify-between items-center gap-4">
        ...
        <div className="flex gap-2 w-full md:w-auto">
            ...
            <button onClick={() => handleOpenModal()} className="flex-1 md:flex-none justify-center bg-blue-600 text-white px-4 py-2 rounded-xl font-bold flex items-center gap-2 hover:bg-blue-700 transition shadow-lg shadow-blue-200"><PlusCircle size={20}/> Novo Membro</button>
        </div>
      </div>
            ...
            <tbody className="divide-y divide-gray-100">
              {currentMembers.map(member => (
                <tr key={member.id} onClick={() => handleOpenModal(member)} className="hover:bg-blue-50/50 transition group cursor-pointer">
                  ...
                  <td className="p-4 text-right"><div className="flex justify-end gap-2">{member.email && <button onClick={(e) => { e.stopPropagation(); openAccessModal(member); }} className="p-2 bg-yellow-50 rounded-lg text-yellow-600 hover:bg-yellow-100 transition"><Key size={16}/></button>}<button onClick={(e) => { e.stopPropagation(); handleOpenModal(member); }} className="p-2 bg-gray-100 rounded-lg text-blue-600 hover:bg-blue-100 transition"><Edit size={16}/></button><button onClick={(e) => { e.stopPropagation(); handleDelete(member.id!); }} className="p-2 bg-gray-100 rounded-lg text-red-600 hover:bg-red-100 transition"><Trash2 size={16}/></button></div></td>
                </tr>
              ))}
            </tbody>
          </table>
