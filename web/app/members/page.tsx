...
export default function MembersPage() {
  ...
  return (
    <div className="p-4 md:p-8 min-h-screen bg-slate-50 pb-24">
      {/* CABEÇALHO */}
      <div className="max-w-6xl mx-auto mb-6 flex flex-col md:flex-row justify-between items-center gap-4">
        <div>
            <h1 className="text-2xl font-bold text-slate-900 flex items-center gap-2"><Users className="text-indigo-600"/> Membros</h1>
        ...
        <div className="flex gap-2 w-full md:w-auto">
            <button onClick={handlePrintExecute} disabled={printing} className="flex-1 md:flex-none justify-center bg-white border border-slate-300 text-slate-700 px-4 py-2 rounded-lg font-bold flex items-center gap-2 hover:bg-slate-100 transition shadow-sm">
                {printing ? <Loader2 className="animate-spin" size={20}/> : <Printer size={20}/>} <span className="hidden md:inline">Imprimir</span>
            </button>
            <button onClick={() => handleOpenEdit()} className="flex-1 md:flex-none justify-center bg-indigo-600 text-white px-4 py-2 rounded-lg font-bold flex items-center gap-2 hover:bg-indigo-700 transition shadow-lg shadow-indigo-500/30"><PlusCircle size={20}/> Novo Membro</button>
        </div>
      </div>

      <div className="max-w-6xl mx-auto mb-6 relative"><Search className="absolute left-3 top-3 text-gray-400" size={20}/><input type="text" placeholder="Buscar por nome..." className="w-full pl-10 p-3 rounded-lg border border-slate-300 outline-none focus:ring-2 focus:ring-indigo-200" value={searchTerm} onChange={e => { setSearchTerm(e.target.value); setCurrentPage(1); }}/></div>
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="hidden md:block bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
            ...
            <table className="w-full text-left border-collapse">
                ...
                <tr><th className="p-4 text-xs font-bold text-slate-500 uppercase">Nome</th><th className="p-4 text-xs font-bold text-slate-500 uppercase">Contato</th><th className="p-4 text-xs font-bold text-slate-500 uppercase text-right">Ações</th></tr>
                </thead>
                <tbody className="divide-y divide-slate-200">
                {currentMembers.map(member => (
                    <tr key={member.id} onClick={() => handleOpenView(member)} className="hover:bg-slate-50 transition group cursor-pointer">
                    <td className="p-4">
                        <div className="flex items-center gap-3">
                            ...
                            <div className="flex flex-col">
                                <span className="font-bold text-slate-800 flex items-center gap-2">
                                    {member.fullName}
                                    {member.permissions && member.permissions.includes('secretary') && <span title="Acesso Secretaria"><Shield size={14} className="text-indigo-500 fill-indigo-100"/></span>}
            ...
            {currentMembers.map(member => (
                <div key={member.id} onClick={() => handleOpenView(member)} className="bg-white p-4 rounded-xl shadow-sm border border-slate-200 flex flex-col gap-3 active:scale-[0.98] transition">
                    ...
                    <div className="grid grid-cols-3 gap-2 mt-2 pt-3 border-t border-slate-100">
                        ...
                        <button onClick={(e) => { e.stopPropagation(); handleOpenEdit(member); }} className="flex items-center justify-center gap-1 py-2 rounded-lg bg-indigo-50 text-indigo-600 font-bold text-xs">
                            <Edit size={14}/> Editar
                        </button>
        ...
        {totalPages > 1 && (<div className="p-6 flex justify-center gap-4 items-center mt-4"><button disabled={currentPage === 1} onClick={() => setCurrentPage(p => p - 1)} className="p-2 rounded-lg hover:bg-slate-100 disabled:opacity-30 bg-white shadow-sm"><ChevronLeft size={20}/></button><span className="text-sm font-bold text-slate-700">Página {currentPage} de {totalPages}</span><button disabled={currentPage === totalPages} onClick={() => setCurrentPage(p => p + 1)} className="p-2 rounded-lg hover:bg-slate-100 disabled:opacity-30 bg-white shadow-sm"><ChevronRight size={20}/></button></div>)}
      </div>
        ...
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4 backdrop-blur-sm animate-in fade-in">
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden animate-in zoom-in-95 relative">
                ...
                <div className="bg-gradient-to-br from-slate-800 to-slate-900 p-8 text-center relative overflow-hidden">
                    ...
                    <div className="w-24 h-24 bg-white rounded-full mx-auto mb-4 flex items-center justify-center border-4 border-slate-700 shadow-lg relative z-10 overflow-hidden">
                        {viewMember.photoUrl ? <img src={getCachedImage(viewMember.photoUrl) || getDirectImageUrl(viewMember.photoUrl)} alt={viewMember.fullName} referrerPolicy="no-referrer" onError={(e) => { e.currentTarget.onerror = null; e.currentTarget.src = `https://ui-avatars.com/api/?name=${encodeURIComponent(viewMember.fullName)}&background=e5e7eb&color=9ca3af`; }} className="w-full h-full object-cover"/> : <User className="text-slate-500" size={48}/>}
                    </div>
                    <h3 className="text-xl font-bold text-white relative z-10">{viewMember.fullName}</h3>
                    <p className="text-slate-400 text-sm uppercase font-bold tracking-wider relative z-10">{translateRole(viewMember.role)}</p>
                ...
                <div className="p-6 space-y-5">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        ...
                        <div className="bg-slate-100 p-3 rounded-lg border border-slate-200 text-center">
                            <span className="text-[10px] text-slate-500 font-bold uppercase">Estado Civil</span>
                            <p className="font-bold text-slate-800 capitalize">{viewMember.maritalStatus === 'single' ? 'Solteiro(a)' : viewMember.maritalStatus === 'married' ? 'Casado(a)' : viewMember.maritalStatus === 'divorced' ? 'Divorciado(a)' : 'Viúvo(a)'}</p>
                        </div>
                    </div>

                    <div className="space-y-3">
                        <div className="flex items-center gap-3 p-3 hover:bg-slate-100 rounded-lg transition">
                            <div className="w-10 h-10 bg-indigo-100 text-indigo-600 rounded-full flex items-center justify-center shrink-0"><Phone size={18}/></div>
                            <div><p className="text-xs text-slate-500 font-bold uppercase">Telefone</p><p className="font-medium text-slate-800">{viewMember.phone || "—"}</p></div>
                        </div>
                        <div className="flex items-center gap-3 p-3 hover:bg-slate-100 rounded-lg transition">
                            ...
                            <div><p className="text-xs text-slate-500 font-bold uppercase">E-mail</p><p className="font-medium text-slate-800 text-sm truncate max-w-[200px]">{viewMember.email || "—"}</p></div>
                        </div>
                        <div className="flex items-center gap-3 p-3 hover:bg-orange-50 rounded-lg transition">
                            ...
                            <div><p className="text-xs text-slate-500 font-bold uppercase">Endereço</p><p className="font-medium text-slate-800 text-sm">{viewMember.address?.street || "—"}</p></div>
                        </div>
                    </div>
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto animate-in zoom-in-95">
                <div className="p-6 border-b border-slate-200 flex justify-between items-center sticky top-0 bg-white z-10">
                    <h2 className="text-xl font-bold text-slate-900">{editingId ? 'Editar Membro' : 'Novo Cadastro'}</h2>
                ...
                <form onSubmit={handleSave} className="p-6 space-y-6">
                    ...
                    <div className="bg-slate-50 p-4 rounded-xl border border-slate-200">
                        <h3 className="text-xs font-bold text-indigo-600 uppercase mb-3 flex items-center gap-2"><Users size={14}/> Dados Pessoais</h3>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div className="md:col-span-2">
                                <label className="text-[10px] font-bold text-slate-500 uppercase">Foto do Membro</label>
                                <div className="grid grid-cols-1 md:grid-cols-3 gap-3 items-center">
                                    <div className="col-span-2">
                                        <div className="flex items-center gap-2">
                                            ...
                                            {photoUploading && <Loader2 className="animate-spin text-indigo-600" size={18}/>}
                                        </div>
                                        <div className="mt-3">
                            ...
                            <div>
                                <label className="text-[10px] font-bold text-slate-500 uppercase">Nascimento</label>
                    ...
                    {/* DADOS ECLESIÁSTICOS */}
                    <div className="bg-slate-50 p-4 rounded-xl border border-slate-200"><h3 className="text-xs font-bold text-indigo-600 uppercase mb-3 flex items-center gap-2"><Building2 size={14}/> Dados Eclesiásticos</h3><div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                        ...
                        <div><label className="text-[10px] font-bold text-slate-500 uppercase">Status</label><select value={formData.status} onChange={e => setFormData({...formData, status: e.target.value})} className="w-full p-3 border rounded-lg bg-white"><option value="active">Ativo</option><option value="inactive">Inativo</option></select></div><div><label className="text-[10px] font-bold text-slate-500 uppercase">Data Batismo</label><input type="date" value={formData.baptismDate} onChange={e => setFormData({...formData, baptismDate: e.target.value})} className="w-full p-3 border rounded-lg bg-white"/></div><div className="flex items-end"><div className="w-full bg-white p-3 rounded-lg border border-amber-200 flex items-center gap-3 cursor-pointer hover:bg-amber-100" onClick={() => setFormData({...formData, isTither: !formData.isTither})}><div className={`w-5 h-5 rounded border flex items-center justify-center ${formData.isTither ? 'bg-amber-500 border-amber-500' : 'border-gray-300'}`}>{formData.isTither && <HandCoins size={12} className="text-white"/>}</div><span className="text-sm font-bold text-gray-700">É Dizimista?</span></div></div></div></div>

                    {/* BLINDAGEM: ÁREA DE PERMISSÃO APENAS PARA ADMIN */}
                    {userRole === 'admin' && (
                        <div className="bg-indigo-50 p-4 rounded-xl border border-indigo-200 animate-in fade-in">
                            <h3 className="text-xs font-bold text-indigo-700 uppercase mb-3 flex items-center gap-2">
                                <ShieldCheck size={14}/> Permissões de Acesso (Login)
                            </h3>
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                <div 
                                    onClick={() => togglePermission('secretary')}
                                    className={`p-3 rounded-lg border flex items-center gap-3 cursor-pointer transition ${formData.permissions.includes('secretary') ? 'bg-indigo-600 text-white border-indigo-600 shadow-lg shadow-indigo-500/30' : 'bg-white text-slate-600 border-slate-200 hover:border-indigo-300'}`}
                                >
                                    <div className={`w-5 h-5 rounded border flex items-center justify-center ${formData.permissions.includes('secretary') ? 'bg-white' : 'bg-gray-100 border-gray-300'}`}>
                                        {formData.permissions.includes('secretary') && <div className="w-3 h-3 bg-indigo-600 rounded-sm" />}
                                    </div>
                                    <div>
                                        <span className="block text-xs font-bold uppercase">Secretaria</span>
                                        <span className={`text-[10px] ${formData.permissions.includes('secretary') ? 'text-indigo-200' : 'text-slate-400'}`}>Acessa e edita membros</span>
                                    </div>
                                </div>

                                <div 
                                    onClick={() => togglePermission('financial')}
                                    className={`p-3 rounded-lg border flex items-center gap-3 cursor-pointer transition ${formData.permissions.includes('financial') ? 'bg-indigo-600 text-white border-indigo-600 shadow-lg shadow-indigo-500/30' : 'bg-white text-slate-600 border-slate-200 hover:border-indigo-300'}`}
                                >
                                    <div className={`w-5 h-5 rounded border flex items-center justify-center ${formData.permissions.includes('financial') ? 'bg-white' : 'bg-gray-100 border-gray-300'}`}>
                                        {formData.permissions.includes('financial') && <div className="w-3 h-3 bg-indigo-600 rounded-sm" />}
                                    </div>
                                    <div>
                                        <span className="block text-xs font-bold uppercase">Tesouraria</span>
                                        <span className={`text-[10px] ${formData.permissions.includes('financial') ? 'text-indigo-200' : 'text-slate-400'}`}>Acessa dízimos e ofertas</span>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <div className="bg-slate-50 p-4 rounded-xl border border-slate-200">
                        <h3 className="text-xs font-bold text-indigo-600 uppercase mb-3 flex items-center gap-2"><Briefcase size={14}/> Ministérios & Departamentos</h3>
                        <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
                            {ministryOptions.map(dept => (
                                <div 
                                    key={dept.id} 
                                    onClick={() => toggleMinistry(dept.id!)}
                                    className={`p-2 rounded-md border text-xs font-bold cursor-pointer transition flex items-center gap-2 ${formData.selectedMinistries.includes(dept.id!) ? 'bg-indigo-600 text-white border-indigo-600' : 'bg-white text-slate-600 border-slate-200 hover:border-indigo-300'}`}
                                >
                    ...
                    <div className="bg-slate-50 p-4 rounded-xl border border-slate-200"><h3 className="text-xs font-bold text-indigo-600 uppercase mb-3 flex items-center gap-2"><MapPin size={14}/> Endereço</h3><div className="grid grid-cols-2 gap-4"><div className="col-span-2"><input placeholder="Rua / Avenida" value={formData.street} onChange={e => setFormData({...formData, street: e.target.value})} className="w-full p-3 border rounded-lg bg-white" /></div><div><input placeholder="Cidade" value={formData.city} onChange={e => setFormData({...formData, city: e.target.value})} className="w-full p-3 border rounded-lg bg-white" /></div><div><input placeholder="Estado/Província" value={formData.state} onChange={e => setFormData({...formData, state: e.target.value})} className="w-full p-3 border rounded-lg bg-white" /></div></div></div>

                    <div className="flex justify-end pt-4 gap-3">
                        <button type="button" onClick={() => setShowModal(false)} className="px-6 py-3 rounded-lg border border-slate-300 text-slate-700 font-bold hover:bg-slate-100">Cancelar</button>
                        <button type="submit" disabled={loading} className="px-6 py-3 rounded-lg bg-indigo-600 text-white font-bold hover:bg-indigo-700 shadow-lg shadow-indigo-500/30">{loading ? 'Salvando...' : 'Salvar Dados'}</button>
                    </div>
                </form>
            </div>
        </div>
