...
import { 
  Users, TrendingUp, ArrowUpRight, ArrowDownRight, Loader2, BarChart3,
  Cake, HandCoins, HeartHandshake, AlertCircle, Sparkles, 
  PieChart as PieChartIcon // Renomeei o ícone para não brigar com o gráfico
} from "lucide-react";
// Importações limpas do Recharts
...
export default function ReportsPage() {
  ...
  const calculateStats = async () => {
    try {
        ...
        // Dizimistas (Baseado no check do cadastro)
        ...
        ].filter(d => d.value > 0); // Só mostra o que tem gente
        ...
        const bdays = members.filter(m => {
            ...
            // Cria data ajustada para evitar erro de fuso (-1 dia)
        ...
        // --- LÓGICA PASTORAL (Sugestão de Visita) ---
        // Pega 3 membros aleatórios para sugerir visita
    ...
    <div className="min-h-screen bg-gray-50 pb-24 font-sans">
      ...
      <div className="max-w-6xl mx-auto px-4 md:px-0 -mt-16 relative z-10">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
              ...
              <div className="bg-white p-5 rounded-3xl shadow-xl shadow-blue-900/5 border border-gray-100 flex items-center gap-4">
              ...
              <div className="bg-white p-5 rounded-3xl shadow-xl shadow-blue-900/5 border border-gray-100 flex items-center gap-4">
                  <div className="p-3 bg-green-50 text-green-600 rounded-full"><HandCoins size={24}/></div>
          ...
          {/* 2. ÁREA PRINCIPAL (GRID 3 COLUNAS) */}
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div className="bg-white p-6 rounded-3xl shadow-xl shadow-blue-900/5 border border-gray-100 flex flex-col h-[420px]">
                  <div className="flex justify-between items-center mb-4 pb-4 border-b border-gray-50">
                      <h3 className="font-bold text-gray-800 flex items-center gap-2"><Cake className="text-pink-500" size={20}/> Aniversariantes</h3>
                      <span className="text-xs font-bold bg-pink-50 text-pink-600 px-2 py-1 rounded-lg capitalize">{currentMonthName}</span>
                  </div>
                  ...
                  <div className="flex-1 overflow-y-auto custom-scrollbar space-y-3 pr-2">
                      {birthdays.length > 0 ? birthdays.map(m => {
                          ...
                          return (
                              <div key={m.id} className={`flex items-center gap-3 p-3 rounded-xl transition ${isToday ? 'bg-pink-50 border border-pink-100' : 'hover:bg-gray-50 border border-transparent'}`}>
                                  <div className={`w-10 h-10 rounded-full flex items-center justify-center font-bold text-sm ${isToday ? 'bg-pink-500 text-white shadow-lg shadow-pink-200' : 'bg-gray-100 text-gray-500'}`}>
                                      {day}
                                  </div>
                                  <div className="flex-1 min-w-0">
                                      <p className={`font-bold text-sm truncate ${isToday ? 'text-pink-700' : 'text-gray-700'}`}>{m.fullName}</p>
                                      <p className="text-[10px] text-gray-400 uppercase">{m.role === 'admin' ? 'Pastor' : 'Membro'}</p>
                                  </div>
                                  {isToday && <Sparkles size={16} className="text-pink-500 animate-pulse"/>}
                              </div>
                          )
                      }) : (
                          <div className="text-center py-10 text-gray-400 flex flex-col items-center">
                              <Cake size={40} className="mb-2 opacity-20"/>
              ...
              {/* COLUNA 2: TERMÔMETRO PASTORAL */}
              <div className="bg-white p-6 rounded-3xl shadow-xl shadow-blue-900/5 border border-gray-100 flex flex-col h-[420px]">
                  <div className="flex justify-between items-center mb-4 pb-4 border-b border-gray-50">
                      <h3 className="font-bold text-gray-800 flex items-center gap-2"><HeartHandshake className="text-blue-500" size={20}/> Visitas Pastorais</h3>
                  </div>
                  ...
                  <div className="mb-6 p-4 bg-blue-50 rounded-2xl border border-blue-100">
                      ...
                      <div className="w-full bg-blue-200 rounded-full h-2 overflow-hidden">
                          <div className="bg-blue-600 h-2 rounded-full w-[40%] transition-all duration-1000"></div>
                      </div>
                  </div>
                  ...
                  <div className="flex-1 overflow-y-auto space-y-2 custom-scrollbar">
                      {needsVisit.map(m => (
                          <div key={m.id} className="flex justify-between items-center p-3 bg-gray-50 border border-gray-100 rounded-xl hover:bg-white hover:shadow-sm transition">
                              <div>
                                  <p className="font-bold text-sm text-gray-700 truncate max-w-[120px]">{m.fullName}</p>
                                  <p className="text-[10px] text-gray-400">Status: {m.status === 'active' ? 'Ativo' : 'Inativo'}</p>
                              </div>
                              <button className="text-[10px] bg-white border border-gray-200 text-gray-600 px-3 py-1.5 rounded-lg font-bold hover:text-blue-600 hover:border-blue-200 transition">
                                  Ver Ficha
                              </button>
                          </div>
                      ))}
              ...
              {/* COLUNA 3: GRÁFICO (PIZZA) */}
              <div className="bg-white p-6 rounded-3xl shadow-xl shadow-blue-900/5 border border-gray-100 flex flex-col h-[420px]">
                  <div className="flex justify-between items-center mb-4 pb-4 border-b border-gray-50">
                      <h3 className="font-bold text-gray-800 flex items-center gap-2"><PieChartIcon className="text-violet-500" size={20}/> Faixa Etária</h3>
                  </div>
                  ...
                  <div className="flex-1 flex flex-col items-center justify-center relative min-h-[200px]">
                      ...
                      <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                          ...
                          <span className="text-[10px] font-bold text-gray-400 uppercase">Membros</span>
                      </div>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-3 mt-4">
                      {ageData.map((d, i) => (
                          <div key={i} className="flex items-center gap-2 text-xs bg-gray-50 p-2 rounded-lg">
                              ...
                              <span className="text-gray-500 font-medium truncate">{d.name}</span>
                              <strong className="text-gray-800 ml-auto">{d.value}</strong>
                          </div>
                      ))}
                  </div>
              </div>
