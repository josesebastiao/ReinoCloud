...
export interface BranchInfo { id: string; name: string; }

interface ChurchContextData {
  user: User | null; churchId: string | null; churchName: string | null;
  userRole: string | null; userPermissions: string[]; userName: string | null;
  loading: boolean; logoUrl: string | null; signatureUrl: string | null; currency: string;
  ...
  hasPermission: (permission: string) => boolean; churchModules: string | null;
  isHeadquarters: boolean; headquartersId: string | null; branches: BranchInfo[];
...
export function ChurchProvider({ children }: { children: React.ReactNode }) {
  ...
  const setChurchData = useCallback((id: string | null, name: string | null, role: string | null, uName: string | null, logo: string | null, signature: string | null, curr: string, modules: string = "full") => {
    setChurchId(id); setChurchName(name); setUserRole(role); setUserName(uName);
    setLogoUrl(logo); setSignatureUrl(signature); setCurrency(curr); setChurchModules(modules);
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, async (currentUser) => {
      ...
      if (currentUser) {
        ...
        if (storedId) {
          setChurchId(storedId); setChurchName(localStorage.getItem("churchName"));
          setUserRole(localStorage.getItem("userRole")); setUserName(localStorage.getItem("userName"));
          setLogoUrl(localStorage.getItem("churchLogo")); setSignatureUrl(localStorage.getItem("churchSignature"));
          setCurrency(localStorage.getItem("churchCurrency") || "AO"); setChurchModules(localStorage.getItem("churchModules") || "full");
        }
        ...
        setChurchId(null); setChurchName(null); setUserRole(null); setUserPermissions([]);
        setUserName(null); setLogoUrl(null); setSignatureUrl(null); setChurchModules("full");
        setIsHeadquarters(false); setHeadquartersId(null); setBranches([]); localStorage.clear();
      }
      setLoading(false);
    });
    return () => unsubscribe();
  ...
  const formatMoney = useCallback((value: number) => {
    return new Intl.NumberFormat(currency === 'BR' ? "pt-BR" : "pt-AO", { style: "currency", currency: currency === 'BR' ? "BRL" : "AOA" }).format(value);
  }, [currency]);
